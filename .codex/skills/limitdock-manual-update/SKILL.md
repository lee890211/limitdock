...
- `scripts/readmodel-ready.go` polls the OpenUsage Unix socket and succeeds only when quota-like rows exist.
- `scripts/capture-limitdock-manual.ps1` writes temporary release-local `settings.json` states, launches the release EXE, waits for OpenUsage readiness, waits for the UI to settle, captures only the LimitDock window/edge region, and creates `manual-slide-in-out.gif` with the bundled Go helper when `go` is available.
